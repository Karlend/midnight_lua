...
local CUTS = {
	...
	["DOOMSDAY"] = function(target, percentage)
		local global_id = CUTS_TBL["DOOMSDAY"][target]
		if not global_id then
			return "Invalid receiver (" .. target .. ")"
		end
		SET_INT_GLOBAL(global_id, percentage or 150)
...
local FEATURES = {
	["P_BAG"] = function(data)
		...
		SET_INT_GLOBAL(262145+29227, bag_balue)
	end,
	["P_FEE"] = function()
	...
	["C_MODPAYOUT"] = function(data)
		...
		--STAT_SET_INT("H3_HARD_APPROACH", 0)
		-- INT
		SET_INT_GLOBAL(1705839,50)
		SET_INT_GLOBAL(1705840,50)
		SET_INT_GLOBAL(1705841,50)
		SET_INT_GLOBAL(1705842,50)
		SET_INT_GLOBAL(262145 + 28303, 10000000)
		SET_INT_GLOBAL(262145 + 28304, 10000000)
		SET_INT_GLOBAL(262145 + 28305, 10000000)
		SET_INT_GLOBAL(262145 + 28306, 10000000)
	end,
	["C_MODCUT"] = function(data)
		SET_INT_GLOBAL(1705839, 100)
		SET_INT_GLOBAL(1705840, 100)
		SET_INT_GLOBAL(1705841, 100)
		SET_INT_GLOBAL(1705842, 100)
	end,
	...
	-- DOOMSDAY
	["D_MODCUT"] = function(data)
		SET_INT_GLOBAL(1699568 + 812 + 50 + 1, 100)
		SET_INT_GLOBAL(1699568 + 812 + 50 + 2, 100)
		SET_INT_GLOBAL(1699568 + 812 + 50 + 3, 100)
		SET_INT_GLOBAL(1699568 + 812 + 50 + 4, 100)
	end,
	-- misc
...
local function RunPreset(info)
	...
	if not need then
		utils.notify("HeistManager", "Preset wasn't found", gui_icon.warning, notify_type.fatal)
		return
	end
	system.fiber(function()
		local problem = need(data)
		if problem then
			utils.notify("HeistManager", problem, gui_icon.warning, notify_type.fatal)
			console.log(con_color.Red, "HeistManager: " .. problem .. "\n")
		else
			utils.notify("HeistManager", "Preset used", gui_icon.world, notify_type.default)
		end
	end)
end
...
local function RunFeature(info)
	...
	if not need then
		utils.notify("HeistManager", "Feature wasn't found", gui_icon.warning, notify_type.fatal)
		return
	end
	system.fiber(function()
		print("Running " .. current)
		local problem = need(data)
		print("Done running feature")
		if problem then
			print("Feature problem")
			utils.notify("HeistManager", problem, gui_icon.warning, notify_type.fatal)
			console.log(con_color.Red, "HeistManager: " .. problem .. "\n")
		else
			print("Successfully ran feature")
			utils.notify("HeistManager", "Feature used", gui_icon.world, notify_type.default)
		end
	end)
end
...
local function RunCut(info)
	...
	if not need then
		utils.notify("HeistManager", "Cut wasn't found", gui_icon.warning, notify_type.fatal)
		return
	end
	...
	system.fiber(function()
		...
		if problem then
			...
			utils.notify("HeistManager", "Cut applied", gui_icon.world, notify_type.default)
		end
	end)
end
